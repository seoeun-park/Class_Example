...
<body>
	...
	<div class="btnSet">
		...
		<!-- 글을 쓴 사용자로 로그인한 경우에만 수정/삭제 가능 -->
		<c:if test="${login_info.id eq vo.writer }">
			<a class="btn-fill" onclick="$('form').attr('action', 'modify.bo'); $('form').submit()">수정</a>
			<a class="btn-fill" onclick="if(confirm('정말 삭제하시겠습니까?')) { $('form').attr('action', 'delete.bo'); $('form').submit() }">삭제</a>
		</c:if>
	</div>
	<div id="popup-background" onclick='$("#popup, #popup-background").css("display", "none");'></div>	<!-- 순서 중요! popup이 맨 위로 가야하기 때문에 -->
	<div id="popup"></div>
	...
	<script type="text/javascript">
		...
		function showAttachImage(id) {
			...
			var ext = filename.substring( filename.lastIndexOf('.') + 1 ).toLowerCase();	//확장자명
			var imgs = ["jpg", "gif", "bmp", "png", "jpeg"];
			if( imgs.indexOf(ext) > -1 ) {
				// /upload/board/2020/09/24/abc.png
				var tag = "<img src='" +  "${vo.filepath}".substring(1)
						+ "' class='" + (id == "#popup" ? "popup" : "file-img") + "'" + "id='preview-img' />";
				$(id).html(tag);
				
			}
		}

		//프리뷰 이미지를 클릭할때 팝업창 띄우기
