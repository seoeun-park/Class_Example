...
<body>
	...
	<div class="btnSet dataOption">
		...
		<a class="btn-empty">병원 조회</a>
		<a class="btn-empty">도서관 조회</a>
	</div>
	<div id="list-top">
		<div>
			<ul>
	...
	<script type="text/javascript">
		$(document).on("click", ".show" , function() {
			...
			var xy = { lat:$(this).data('y') , lng:Number($(this).data('x')) };
		...
		function pharmacy_grid_data( data, type ) {
			...
			if(type == 0) {
				data.each(function() {
					tag += "<li>"
						+ "<div><a class='show' data-x=" + this.XPos + " data-y= " + this.YPos + ">" + this.yadmNm + "</a></div>"
						+ "<div>" + this.telno + "</div>"
						+ "<div>" + this.addr + "</div>"
						+ "</li>";
				});
				...
				data.each(function() {
					if( $(this).index() > 0 ) {
						var $a = $(this).find(".show");
						tag += "<li>"
							+ "<div><a class='show' data-x=" + $a.data("x") + " data-y= " + $a.data("y") + ">" + $(this).children("td:eq(0)").text() + "</a></div>"
		...
		function pharmacy_list(page) {
			$.ajax({
				...
				success: function( data ){
					...
					// 페이지 전환을 해도 뷰의 형태(리스트, 그리드)가 유지되도록 함
		...
		function makePage(totalList, curPage) {
			...
			pageVO.totalPage = parseInt(totalList / pageList) + (totalList % pageList > 0 ? 1 : 0);
			pageVO.totalBlock = parseInt(pageVO.totalPage / blockPage) + (pageVO.totalPage % blockPage == 0 ? 0 : 1);
			pageVO.curBlock = parseInt(curPage / blockPage) + (curPage % blockPage == 0 ? 0 : 1);
