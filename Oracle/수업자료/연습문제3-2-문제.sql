...
SELECT first_name || ' ' || last_name name,
               LENGTH(first_name || last_name) "이름의 길이"
FROM employees
...
SELECT first_name || ' ' || last_name name, LPAD(salary, 15, '$') salary
FROM employees
...
SELECT employee_id, first_name || ' ' || last_name name, department_id, phone_number,
               SUBSTR(phone_number, 1, INSTR(phone_number, '.') - 1) "local number",
               SUBSTR(phone_number, INSTR(phone_number, '.', -1) + 1) "private number"
FROM employees
